...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        with vs.variable_scope("firstModelLayer"):
            modeling_layer_encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
            m_intermediate = modeling_layer_encoder.build_graph(blended_reps, self.context_mask)
        with vs.variable_scope("secondModelLayer"):
            modeling_layer_encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
            m = modeling_layer_encoder.build_graph(m_intermediate, self.context_mask)
	    ...
	    output_layer_encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
            m2 = output_layer_encoder.build_graph(m, self.context_mask)
